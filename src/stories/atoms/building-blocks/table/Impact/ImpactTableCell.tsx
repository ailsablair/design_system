...
export interface ImpactTableCellProps {
  ...
  width?: 'lg' | 'sm';
  /** State of the cell */
  state?: 'default' | 'empty' | 'disabled' | 'disabled-empty' | 'decimal' | 'text' | 'scale';
  /** Hover state */
  hover?: boolean;
  /** Cell content/text */
      ...
      <div className={cellClasses}>
        <div className="content">
          <div className="category-lock">
            ...
            {showLock && (
              <svg className="filled-icons" width="18" height="18" viewBox="0 0 18 18" fill="none">
  ...
  if (type === 'header-w-subtext') {
    if (width === 'sm') {
      return (
        <div className={cellClasses}>
          <div className="content">
            <div className="_2025">{children || '2025'}</div>
            <div className="_0-a">{subtitle || '0A'}</div>
          </div>
        </div>
      );
    }

...
export interface AccordionHeaderProps {
  ...
  /** Icon style for expand/collapse - matches Figma "open-icon" variants */
  openIcon?: 'default (chevron)' | 'plus';
  /** Type variant - matches main accordion types */
  ...
  /** Accessibility label */
  'aria-label'?: string;
  /** Whether this header is interactive */
  interactive?: boolean;
}
 ...
 *
 * Standalone accordion header that can be used independently or as part of larger accordion systems.
 * Supports all Figma variants with pixel-perfect styling and full accessibility.
 *
 * Features:
 * - 5 type variants: simple, decorative, number, label, no-stroke
 * - 3 sizes with exact Figma typography
 * - 2 icon styles: chevron and plus/minus
...
export const AccordionHeader: React.FC<AccordionHeaderProps> = ({
  ...
  // Generate CSS classes - matching Figma design system
  const headerClasses = [
    'accordion-header',
    `accordion-header--type-${type}`,
    `accordion-header--size-${size}`,
    `accordion-header--open-icon-${openIcon === 'default (chevron)' ? 'chevron' : openIcon}`,
    open ? 'accordion-header--open' : 'accordion-header--closed',
    `accordion-header--state-${state}`,
    interactive && 'accordion-header--interactive',
    className
  ].filter(Boolean).join(' ');
  ...
  const StatusIcon = () => {
    ...
    return (
      <div className="accordion-header__status-icon">
        <div className="accordion-header__status-empty">
  ...
  const renderHeaderContent = () => {
    switch (type) {
      case 'simple':
        return (
          <>
            ...
            <div className="accordion-header__heading-content">
              <div className="accordion-header__title">{title}</div>
              {description && <div className="accordion-header__description">{description}</div>}
            </div>
            <div className="accordion-header__toggle-icon">{getToggleIcon()}</div>
          </>
        );
      ...
      case 'decorative':
        return (
          <>
            <div className="accordion-header__icon-frame">
              {icon || <BellRingIcon />}
            </div>
            <div className="accordion-header__title">{title}</div>
            <div className="accordion-header__toggle-icon">{getToggleIcon()}</div>
      ...
      case 'number':
        return (
          <>
            <div className="accordion-header__number">{number}</div>
            <div className="accordion-header__title">{title}</div>
            <div className="accordion-header__toggle-icon">{getToggleIcon()}</div>
          </>
        );
      ...
      case 'label':
        return (
          <>
            <div className="accordion-header__title">{title}</div>
            {label && <div className="accordion-header__label-container">{label}</div>}
            <div className="accordion-header__toggle-icon">{getToggleIcon()}</div>
          </>
        );

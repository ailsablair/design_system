/**
 * StatusIconwLabel Assets
 * 
 * Re-exports from the StatusIcon asset library to ensure consistent icon paths
 * and provides the status-to-icon mapping for all 8 status types.
 */

import { statusIconPaths, type StatusIconType } from '../status-icons/statusIcon.assets';
import { STATUS_ICON_MAP, STATUS_COLORS, TOKENS } from './statusIconwLabel.styles';
import type { StatusIconwLabelStatus } from './statusIconwLabel.types';

/**
 * Re-export the icon paths for direct access
 */
export { statusIconPaths, type StatusIconType } from '../status-icons/statusIcon.assets';

/**
 * Re-export style helpers and mappings
 */
export { STATUS_ICON_MAP, STATUS_COLORS, TOKENS, getSizeConfig, getStepTheme } from './statusIconwLabel.styles';

/**
 * Gets the icon path for a given status type
 */
export function getIconPathForStatus(status: StatusIconwLabelStatus): React.ReactNode {
  const iconType = STATUS_ICON_MAP[status];
  return statusIconPaths[iconType];
}

/**
 * Gets the color configuration for a given status type
 */
export function getColorsForStatus(status: StatusIconwLabelStatus) {
  return STATUS_COLORS[status];
}

/**
 * Backward compatibility: Legacy image-based assets
 * Kept for backward compatibility with older implementations
 * @deprecated Use inline SVG rendering with getIconPathForStatus instead
 */
export type StatusIconwLabelStatus_Legacy = 'warning';
...
export type StatusIconwLabelAssetMap = Record<StatusIconwLabelStatus_Legacy, StatusIconwLabelAssetStateMap>;
...
export const getStatusIconwLabelAsset = (
  status: StatusIconwLabelStatus_Legacy,

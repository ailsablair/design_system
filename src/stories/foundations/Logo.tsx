...
export interface LogoProps {
  ...
  color?: 'brand' | 'brand-dark' | 'dark-blue' | 'sky-blue' | 'white' | 'black';
  /** Size of the logo */
...
const LogomarkSVG = ({ color, width = 290, height = 280 }: { color: string; width: number; height: number }) => {
  const getFillColors = () => {
    switch (color) {
      case 'white':
  ...
  // Calculate viewBox based on the original Figma design
  ...
  return (
    ...
    >
      {/* Main outer curve */}
      <path 
        d="M167.352 259.988C115.327 259.988 71.1049 220.681 65.4611 168.311L64.231 156.917L156.306 152.083L151.241 55.1626L162.058 54.5947C218.497 51.6101 266.891 95.2553 269.858 151.842C272.813 208.428 229.278 256.882 172.803 259.855C170.982 259.952 169.161 260 167.364 260L167.352 259.988ZM89.0375 177.338C98.3956 214.192 132.681 240.207 171.657 238.165C216.205 235.833 250.551 197.614 248.223 152.965C246.077 112.003 213.781 79.6556 174.045 76.4052L179.074 172.601L89.0375 177.326V177.338Z" 
        fill={colors.primary}
      />
      {/* Inner left curve */}
      <path 
        d="M21.275 117.199L20.467 109.538C20.3102 108.052 20.1896 106.553 20.1052 105.055C19.0198 83.5344 26.3641 62.8718 40.7752 46.8855C55.1984 30.8992 74.964 21.4983 96.442 20.4108L104.558 20L109.237 112.74L21.275 117.199ZM89.1701 37.5813C75.1931 40.0584 62.4944 47.0788 52.8227 57.7968C42.2344 69.5418 36.4217 84.4406 36.2408 100.137L92.185 97.2973L89.1701 37.5692V37.5813Z" 
        fill={colors.secondary}
      />
      {/* Inner right curve */}
      <path 
        d="M50.3022 144.181L49.3616 135.239C49.2048 133.729 49.0722 132.206 48.9998 130.672C47.8903 108.789 55.3552 87.7877 70.0196 71.5235C84.684 55.2714 104.787 45.7013 126.615 44.6018L136.082 44.1184L140.905 139.565L50.3022 144.157V144.181ZM118.137 64.7689C105.053 67.4272 93.1859 74.1577 84.0689 84.2715C74.0233 95.4123 68.3795 109.441 67.873 124.28L121.007 121.585L118.137 64.7689Z" 
...
const FullLogoSVG = ({ color, width = 1187, height = 436 }: { color: string; width: number; height: number }) => {
  const getColors = () => {
    switch (color) {
      case 'white':
        return { logomark: 'white', wordmark: 'white' };
      case 'sky-blue':
        return { logomark: 'sky-blue', wordmark: '#0BA7EA' };
      case 'dark-blue':
        return { logomark: 'dark-blue', wordmark: '#191E3C' };
      case 'black':
        return { logomark: 'black', wordmark: '#1C1C1C' };
      case 'brand-dark':
        return { logomark: 'brand', wordmark: '#1C1C1C' };
      case 'brand':
      default:
        return { logomark: 'brand', wordmark: 'white' };
    }
  };

  const colors = getColors();
  
  // Calculate proportional dimensions based on the Figma design
  const logomarkWidth = Math.round(width * 0.25);  // Logomark takes about 25% of width
  const logomarkHeight = Math.round(height * 0.64); // Maintain aspect ratio
  const wordmarkWidth = Math.round(width * 0.67);   // Wordmark takes about 67% of width
  const wordmarkHeight = Math.round(height * 0.39);  // Maintain aspect ratio
  
  const gap = Math.round(width * 0.02); // 2% gap between logomark and wordmark
...
export const Logo = ({
  ...
  const getSizeDimensions = () => {
    ...
    const sizeMap = {
      small: { 
        logomark: { width: 116, height: 112 }, 
        wordmark: { width: 272, height: 67 }, 
        full: { width: 475, height: 174 } 
      },
      medium: { 
        logomark: { width: 174, height: 168 }, 
        wordmark: { width: 408, height: 101 }, 
        full: { width: 712, height: 262 } 
      },
      large: { 
        logomark: { width: 232, height: 224 }, 
        wordmark: { width: 544, height: 134 }, 
        full: { width: 950, height: 349 } 
      }
    };
